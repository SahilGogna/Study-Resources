# 30-Day Tech Job Search Roadmap (Canada)

**For:** Software Engineers | Data Scientists | Data Analysts | Data Engineers | Cloud Engineers | DevOps Engineers

**Time Commitment:** 4 hours/day  
**Approach:** Upskilling-Heavy

---

## Target Companies (Canada)

### Product/Tech Companies
Shopify • Amazon • Microsoft • Google • Meta • Cisco • IBM • Oracle • SAP • Salesforce • Snowflake • Databricks • Stripe • Square • Wealthsimple • Unity • Hootsuite • Lightspeed • Clio

### Finance/Banking (Tech Divisions)
TD Bank (Tech) • RBC (Innovation) • Scotiabank (Digital) • BMO (Technology) • CIBC (Digital) • Manulife (Tech)

### Telecom
Telus Digital • Rogers • Bell

---

## MONDAY/TUESDAY (4 Hours)

### LinkedIn Optimization (1 hour)
✓ Update headline with role-specific keywords  
✓ Optimize About section with measurable achievements  
✓ Add projects to Featured section  
✓ Align Skills with target job descriptions  

**Tools:**
- Jobscan: https://www.jobscan.co/
- LinkedIn Profile Optimization (built-in)

---

### Proactive Networking (1 hour)
✓ Connect with hiring managers, recruiters, engineers  
✓ Engage with company tech blogs and posts  
✓ Request informational interviews  

---

### Upskilling - YouTube Channels (2 hours)

#### Software Engineering
- **NeetCode:** https://www.youtube.com/@NeetCode
- **Tech With Tim:** https://www.youtube.com/@TechWithTim
- **Web Dev Simplified:** https://www.youtube.com/@WebDevSimplified
- **Fireship:** https://www.youtube.com/@Fireship
- **ThePrimeagen:** https://www.youtube.com/@ThePrimeagen
- **Code with Mosh:** https://www.youtube.com/@programmingwithmosh

#### Data Science
- **StatQuest with Josh Starmer:** https://www.youtube.com/@statquest
- **Krish Naik:** https://www.youtube.com/@krishnaik06
- **CampusX:** https://www.youtube.com/@campusx-official
- **Ken Jee:** https://www.youtube.com/@KenJee_ds
- **Data Professor:** https://www.youtube.com/@DataProfessor
- **Unfold Data Science:** https://www.youtube.com/@unfolddata

#### Data Analytics
- **Alex The Analyst:** https://www.youtube.com/@AlexTheAnalyst
- **Luke Barousse:** https://www.youtube.com/@LukeBarousse
- **Data with Mo:** https://www.youtube.com/@datawithmo
- **Analyst Builder:** https://www.youtube.com/@AnalystBuilder
- **Data Science Jay:** https://www.youtube.com/@DataScienceJay

#### Data Engineering
- **Seattle Data Guy:** https://www.youtube.com/@SeattleDataGuy
- **E-learning Bridge:** https://www.youtube.com/@shashank_mishra
- **Darshil Parmar:** https://www.youtube.com/@DarshilParmar
- **Data Engineering Simplified:** https://www.youtube.com/@DataEngineeringSimplified

#### Cloud Engineering
- **TechWorld with Nana:** https://www.youtube.com/@TechWorldwithNana
- **KodeKloud:** https://www.youtube.com/@KodeKloud
- **A Cloud Guru:** https://www.youtube.com/@AcloudGuru
- **AWS Official:** https://www.youtube.com/@amazonwebservices
- **Google Cloud Tech:** https://www.youtube.com/@GoogleCloudTech

#### DevOps
- **TechWorld with Nana:** https://www.youtube.com/@TechWorldwithNana
- **DevOps Toolkit:** https://www.youtube.com/@DevOpsToolkit
- **KodeKloud:** https://www.youtube.com/@KodeKloud
- **Travis Media:** https://www.youtube.com/@TravisMedia
- **DevOps Journey:** https://www.youtube.com/@DevOpsJourney

---

## WEDNESDAY/THURSDAY (4 Hours)

### Technical Interview Prep (2 hours)

#### DSA & Coding Practice
- **LeetCode:** https://leetcode.com/ (Follow NeetCode 150 or Blind 75)
- **HackerRank:** https://www.hackerrank.com/
- **CodeSignal:** https://codesignal.com/
- **Grind 75:** https://www.techinterviewhandbook.org/grind75
- **freeCodeCamp:** https://www.freecodecamp.org/
- **The Odin Project:** https://www.theodinproject.com/

#### Role-Specific Technical Prep

**SQL (DS/DA/DE):**
- **DataLemur:** https://datalemur.com/
- **SQLBolt:** https://sqlbolt.com/
- **Mode Analytics SQL Tutorial:** https://mode.com/sql-tutorial/

**System Design (SWE):**
- **ByteByteGo YouTube:** https://www.youtube.com/@ByteByteGo
- **System Design Primer (GitHub):** https://github.com/donnemartin/system-design-primer
- **System Design Interview (Book):** https://www.amazon.ca/System-Design-Interview-insiders-Second/dp/B08CMF2CQF

**Cloud Hands-on (Cloud/DevOps):**
- **AWS Free Tier:** https://aws.amazon.com/free/
- **Google Cloud Skills Boost:** https://www.cloudskillsboost.google/
- **AWS Cloud Quest (Gamified):** https://aws.amazon.com/training/digital/aws-cloud-quest/
- **Azure Free Account:** https://azure.microsoft.com/free/

---

### Behavioral Interview Prep (1 hour)

**Mock Interview Platforms:**
- **Pramp:** https://www.pramp.com/
- **Interviewing.io:** https://interviewing.io/

**AI-Powered Practice:**
- **Yoodli:** https://www.yoodli.ai/
- **ChatGPT:** https://chat.openai.com/ (STAR method practice)

**Resources:**
- **Exponent:** https://www.tryexponent.com/
- **IGotAnOffer:** https://igotanoffer.com/
- Amazon's 14 Leadership Principles
- Meta's 5 Core Values

---

### Automate Job Search (1 hour)

#### Resume Optimization Tools
- **Rezi AI:** https://www.rezi.ai/
- **Resume Worded:** https://resumeworded.com/
- **Jobscan:** https://www.jobscan.co/
- **Kickresume:** https://www.kickresume.com/
- **Enhancv:** https://enhancv.com/

#### Job Matching & Auto-Apply
- **Simplify:** https://simplify.jobs/ (Chrome Extension - Auto Apply)
- **Teal:** https://www.tealhq.com/
- **LazyApply:** https://lazyapply.com/
- **Sonara:** https://sonara.ai/

#### Application Tracking
- **Huntr:** https://huntr.co/
- **Notion Templates:** https://www.notion.so/templates/job-search
- **Airtable Templates:** https://www.airtable.com/templates

#### Company Research Tools
- **ChatGPT:** https://chat.openai.com/
- **Perplexity AI:** https://www.perplexity.ai/
- **Glassdoor:** https://www.glassdoor.ca/
- **Blind:** https://www.teamblind.com/
- **Levels.fyi:** https://www.levels.fyi/

---

## FRIDAY (4 Hours)

### AI Fundamentals & Domain Application (2-3 hours)

#### Learning AI Basics
- **Andrew Ng - AI For Everyone (Coursera):** https://www.coursera.org/learn/ai-for-everyone
- **DeepLearning.AI Short Courses:** https://www.deeplearning.ai/short-courses/
- **ChatGPT Prompt Engineering for Developers:** https://www.deeplearning.ai/short-courses/chatgpt-prompt-engineering-for-developers/
- **Elements of AI:** https://www.elementsofai.com/

#### AI Tools by Domain

**Software Engineering:**
- **GitHub Copilot:** https://github.com/features/copilot
- **Cursor AI:** https://cursor.sh/
- **Tabnine:** https://www.tabnine.com/
- **ChatGPT for debugging:** https://chat.openai.com/

**Data Science/Analytics:**
- **Julius AI:** https://julius.ai/
- **ChatGPT for data analysis prompts:** https://chat.openai.com/
- **DataChat:** https://datachat.ai/
- **Pandas AI:** https://github.com/gventuri/pandas-ai

**Data Engineering:**
- **AI for ETL optimization**
- **dbt automation:** https://www.getdbt.com/
- **Data quality checks with AI**

**Cloud/DevOps:**
- **AI for Infrastructure as Code**
- **ChatGPT for Terraform/CloudFormation**
- **AI-powered monitoring tools**

#### Hands-on AI Projects
- Build a ChatGPT-powered tool for your domain
- Automate workflows using n8n + OpenAI API
- Create a portfolio project with AI integration

---

### Resume Tools Deep Dive (1 hour)

**Test Multiple Platforms:**
- **Rezi:** https://www.rezi.ai/
- **Resume Worded:** https://resumeworded.com/
- **Kickresume:** https://www.kickresume.com/
- **Enhancv:** https://enhancv.com/
- **Overleaf (LaTeX):** https://www.overleaf.com/ (For tech resumes)

**ATS Optimization:**
- **Jobscan:** https://www.jobscan.co/

**Portfolio Integration:**
- Optimize GitHub README
- Update personal website/portfolio

---

## SATURDAY/SUNDAY (8 Hours Total)

### Deep Upskilling (5-6 hours)

#### Structured Learning Paths

**Software Engineering:**
- **The Odin Project:** https://www.theodinproject.com/
- **freeCodeCamp:** https://www.freecodecamp.org/
- **CS50 (Harvard):** https://cs50.harvard.edu/x/
- **Full Stack Open:** https://fullstackopen.com/

**Data Science:**
- **Kaggle Learn:** https://www.kaggle.com/learn
- **DataCamp (Free Tier):** https://www.datacamp.com/
- **Fast.ai:** https://www.fast.ai/
- **Google Data Analytics Certificate:** https://www.coursera.org/professional-certificates/google-data-analytics

**Data Analytics:**
- **Google Data Analytics Certificate:** https://www.coursera.org/professional-certificates/google-data-analytics
- **Maven Analytics:** https://www.mavenanalytics.io/
- **Mode Analytics Tutorial:** https://mode.com/sql-tutorial/

**Data Engineering:**
- **DataTalksClub:** https://datatalks.club/
- **Data Engineering Zoomcamp:** https://github.com/DataTalksClub/data-engineering-zoomcamp
- **Fundamentals of Data Engineering (Book):** https://www.oreilly.com/library/view/fundamentals-of-data/9781098108298/

**Cloud Engineering:**
- **AWS Cloud Quest:** https://aws.amazon.com/training/digital/aws-cloud-quest/
- **Google Cloud Skills Boost:** https://www.cloudskillsboost.google/
- **Azure Learn:** https://learn.microsoft.com/en-us/training/azure/

**DevOps:**
- **KodeKloud:** https://kodekloud.com/
- **DevOps Bootcamp:** https://www.udemy.com/course/devops-bootcamp/
- **Kubernetes Official Docs:** https://kubernetes.io/docs/tutorials/

---

### Building Projects (2-3 hours)

#### Project Ideas by Role

**Software Engineering:**
- Full-stack CRUD application (MERN/PERN stack)
- REST API with authentication (JWT)
- Chrome extension for productivity
- Real-time chat application (WebSockets)
- Mobile app (React Native/Flutter)

**Data Science:**
- End-to-end ML project on Kaggle dataset
- Deployed model using Streamlit/Gradio
- Time series forecasting project
- NLP sentiment analysis application
- Computer vision project (image classification)

**Data Analytics:**
- Interactive dashboard (Tableau/Power BI/Streamlit)
- SQL case study with complex queries
- A/B testing analysis with statistical significance
- Business metrics analysis project
- Automated reporting pipeline

**Data Engineering:**
- ETL pipeline using Apache Airflow + dbt
- Real-time data streaming with Kafka
- Data warehouse design (Snowflake/BigQuery)
- API data ingestion automation
- Data quality monitoring system

**Cloud Engineering:**
- Multi-tier AWS/GCP architecture deployment
- Terraform infrastructure as code project
- Serverless application (AWS Lambda/Cloud Functions)
- Container orchestration (EKS/GKE)
- Cloud cost optimization analysis

**DevOps:**
- CI/CD pipeline (GitHub Actions/Jenkins)
- Kubernetes cluster deployment with Helm
- Monitoring stack (Prometheus + Grafana)
- Infrastructure as Code (Terraform/Ansible)
- Dockerized multi-container application

---

#### Project Resources

**Project Inspiration:**
- **GitHub - Build Your Own X:** https://github.com/codecrafters-io/build-your-own-x
- **App Ideas Collection:** https://github.com/florinpop17/app-ideas
- **Roadmap.sh Projects:** https://roadmap.sh/projects
- **DevProjects by Codementor:** https://www.codementor.io/projects

**Portfolio Hosting:**
- **GitHub Pages:** https://pages.github.com/
- **Vercel:** https://vercel.com/
- **Netlify:** https://www.netlify.com/
- **Railway:** https://railway.app/

---

## Weekly Review Checklist

✓ Applications sent this week: _____  
✓ Responses received: _____  
✓ Interviews scheduled: _____  
✓ Hours spent upskilling: _____  
✓ New connections made: _____  
✓ Projects worked on: _____  

---

## Key Metrics to Track

- **Response Rate:** (Responses / Applications) × 100
- **Interview Conversion:** (Interviews / Responses) × 100
- **Networking Effectiveness:** Replies from cold outreach
- **Skills Progress:** Courses completed, projects built

---

## Pro Tips

1. **Consistency > Intensity:** 4 focused hours daily beats 10 random hours
2. **Quality > Quantity:** 5 tailored applications beat 50 generic ones
3. **Build in Public:** Share your learning journey on LinkedIn
4. **Network Genuinely:** Help others, don't just ask for referrals
5. **Document Everything:** Every project is portfolio material
6. **Mental Health Matters:** Take breaks, job search is a marathon

---

**Remember:** December and early January see slower hiring. Use this time strategically to upskill and position yourself for the hiring surge in late January/February.

**Good luck with your job search!**


---

### Want to Build Real Analytics Projects?

ORU's **Hands-On Data Analytics** course teaches you to think like a practitioner and execute like one — through realistic business problems across insurance, banking, fintech, and ecommerce. Tools covered include Python, SQL, BigQuery, dbt Core, Tableau, Power BI, Streamlit, and Vanna AI.

Use code **ORU10** for 10% off → [Enroll Now](https://joinoru.com/course/hands-on-data-analytics:-learn-by-building-industry-projects)

*For mentorship, live sessions, and community support: [joinoru.com](https://joinoru.com)*
